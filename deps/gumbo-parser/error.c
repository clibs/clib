...
static int print_message(GumboParser *parser, GumboStringBuffer *output,
  ...
  if (bytes_written > remaining_capacity) {
    gumbo_string_buffer_reserve(parser, output->capacity + bytes_written,
                                output);
    remaining_capacity = output->capacity - output->length;
    bytes_written = vsnprintf(output->data + output->length, remaining_capacity,
                              format, args);
  }
...
static const char *find_last_newline(const char *original_text,
...
static const char *find_next_newline(const char *original_text,
                                     const char *error_location) {
  const char *c = error_location;
  for (; *c && *c != '\n'; ++c)
    ;
...
void gumbo_error_to_string(GumboParser *parser, const GumboError *error,
                           GumboStringBuffer *output) {
  print_message(parser, output, "@%d:%d: ", error->position.line,
...
void gumbo_caret_diagnostic_to_string(GumboParser *parser,
  ...
  gumbo_string_buffer_append_codepoint(parser, '\n', output);
  gumbo_string_buffer_reserve(parser, output->length + error->position.column,
                              output);
...
void gumbo_print_caret_diagnostic(GumboParser *parser, const GumboError *error,
                                  const char *source_text) {
